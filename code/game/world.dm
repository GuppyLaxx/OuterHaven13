...
/world/proc/CheckSecurityMode()
	...
	if(!text2file("The world is running at least safe mode", "data/server_security_check.lock"))
		...
		warning("Outer Haven is not supported in ultrasafe security mode. Everything will break!")
		return
	...
	if(shell("echo \"The world is running in trusted mode\"") != null)
		...
		warning("Outer Haven uses many file operations a few shell()s, and some external call()s. Trusted mode is recommended. You can download our source code for your own browsing and compilation at https://github.com/OuterHaven13/OuterHaven13")
